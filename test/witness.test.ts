...
import { Matrix as MatrixGroth16, NoInputs } from "./fixture-projects/complex-circuits/generated-types/zkit";
  ...
  describe("groth16", () => {
    ...
    describe("witnessOutputs", () => {
      ...
      it("should not pass if outputs are incorrect for given inputs", async () => {
        await expect(
          expect(matrix).with.witnessInputs({ a, b, c }).to.have.witnessOutputs({ e: d }),
        ).to.be.rejectedWith(
      ...
      it("should not pass if negated but outputs are correct", async () => {
        ...
        ).to.be.rejectedWith(`Expected output signal "d" NOT to be "[[2,5,0],[17,26,0],[0,0,0]]", but it is"`);
      });
